...
export class BookingServiceImpl implements BookingService { // Implement the interface (if using)
  ...
  createBooking(bookingDto: BookingDTO): Observable<ApiResponse<BookingDTO>> {
    // Note: Do not include bookingId or bookingTime in the DTO for creation,
    // as they are typically generated by the backend/database.
    const payload: BookingDTO = { ...bookingDto }; // Create a copy to avoid modifying the original DTO
    delete payload.bookingId; // Remove bookingId for creation
    delete payload.bookingTime; // Remove bookingTime for creation (backend uses @CreationTimestamp)

    return this.http.post<ApiResponse<BookingDTO>>(`${this.apiUrl}`, payload).pipe(
  ...
  // Example: getBookingsByCustomerId(customerId: number): Observable<ApiResponse<BookingDTO[]>> { ... }
  // Example: updateBookingStatus(bookingId: number, status: BookingStatus): Observable<ApiResponse<BookingDTO>> { ... }
}
